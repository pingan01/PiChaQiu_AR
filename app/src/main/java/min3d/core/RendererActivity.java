...
/**
 * 创建要使用min3d的Activity时，继承这个类。
 * 为实现最基本的逻辑，覆写initScene()和updateScene()函数。
 * 如果需要改变布局的话，覆写onCreateSetContentView()。参看例子：“3D inside layout”
 * 如果需要在UI主线程中改变3d场景中的相关变量，覆写onUpdateScene()和onUpdateScene()。
 */
public class RendererActivity extends Activity implements ISceneController
{
    ...
    {
		// 例如：配合setting scene.backgroundColor to 0x0，可使glSurfaceView变得透明
